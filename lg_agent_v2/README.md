...
checkout to a confirmed order — built on **LangChain + LangGraph** (`create_agent`).
It is a clean-room port of [`pydantic_agent_v1`](../pydantic_agent_v1): the **same
three-layer architecture** and the **same frontend wire contract**, with only the
harness swapped from Pydantic AI to LangChain. Read both side by side to see exactly
...
> reply; the **cart** is the single source of truth and the only thing that can say an
> order was placed.
...
# point the frontend at it:  AGENT_V2_API_URL=http://localhost:8006

# tests (26, fully offline — agents driven by scripted fake chat models)
uv run pytest tests_lg_agent_v2 -q
```

**Model.** Resolved from the env: `LG_AGENT_V2_MODEL` → `AGENT_V2_OPENAI_MODEL` →
`openai:gpt-5.4-mini`. Needs `OPENAI_API_KEY` (the package loads the repo `.env` on
import).

**Verified end to end (real gpt-5.4-mini):** full checkout — add → identity → address +
serviceability → 2h delivery (auto shipping + tax quote, total **$74.35**) → cash →
ready → `yes` → receipt `RCPT-9000`; the writer streams tokens throughout. This is the
exact same flow and total as the pydantic build — same spec, swapped harness.
