...
function; here it is a :func:`~lg_agent_v2.runtime.dynamic_instructions` middleware —
...
from lg_agent_v2.agents.names import CHECKOUT
...
def build(model: Any | None = None):
    """Compile the checkout worker (optionally with an injected model, for tests)."""
    return create_agent(
        ...
        middleware=[dynamic_instructions(_progress_anchor), no_parallel_tools()],
        name=CHECKOUT,
    )
