...
The whole config → ``create_agent`` mapping lives here:
  ...
  * prompt     → system_prompt + instructions appended as bullet rules
  ...
  * output_format → ``response_format`` (raw JSON-schema dict, verified accepted)

Middleware order mirrors v2/v4: ``[skills?, *guardrails, *middleware]``.
``context_schema`` / ``checkpointer`` / ``store`` are runtime args, not config.
"""
...
from agent_v4_1.core.config import AgentConfig, CustomSkillSpec, RegistrySkillSpec, to_config
from agent_v4_1.core.guardrails import compile_guardrail_middleware
from agent_v4_1.core.models import resolve_model
from agent_v4_1.core.registry import MIDDLEWARE, SKILLS
from agent_v4_1.core.skills import Skill, SkillsMiddleware
from agent_v4_1.core.tools import resolve_tools
...
def build_agent(
    ...
    *,
    ...
    time rather than declared in the config. They are a *runtime* wiring concern
    (like ``context_schema`` / ``store``) — a sub-agent carries Python extractors,
    so it is not pure-JSON config and does not belong in ``config.tools``. This is
    what makes an orchestrator: ``build_agent(ORCHESTRATOR, delegates=[...])``.
    """
