...
One generic :class:`Registry` backs all four singletons. Unlike agent_v4's
registry it *raises* on a duplicate name (pass ``replace=True`` to override
intentionally) — a silent overwrite is a debugging trap. The shopping platform
populates these in :func:`agent_v4_1.shopping.platform.register_shopping_platform`;
the EXAMPLE config's tools are stubbed by tests.
"""
...
# The four registries. Types are loose (Any) to avoid importing langchain here.
