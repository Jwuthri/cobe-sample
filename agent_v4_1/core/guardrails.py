...
Two consumers, one set of rule objects (so they can't drift):

  * :func:`compile_guardrail_middleware` — attach rules as middleware to a
    sub-agent / writer via ``build_agent`` (output-side checks live here).
  * :func:`compile_input_rules` + :func:`run_input_guardrails` — run the SAME
    middleware instances' ``before_model`` as a pre-flight step in the turn
    pipeline, BEFORE the orchestrator's first model call. A refusal is an
    instant event; a redaction rewrites the user text before it enters the
    transcript, so every downstream model sees only clean input.

Three rule types are registered: ``pii`` (built-in ``PIIMiddleware``),
``blocklist`` (phrase/regex), and ``llm_judge`` (a small model judges a
natural-language policy). The blocklist/judge classes short-circuit a run with
``{"jump_to": "end"}`` and replace a violating output **by message id** (so the
offending text never lingers in state).
"""
...
from agent_v4_1.core.registry import GUARDRAILS
