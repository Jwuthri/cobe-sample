...
This is the contract :mod:`agent_v4_1.core.config` is designed around — the
...
shopping platform doesn't register, so it validates always but only *builds*
once those tools are stubbed (see ``tests_v4_1/test_factory.py``). Don't "fix"
the missing tools — that's the point.
"""
