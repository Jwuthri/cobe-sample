...
A config's ``tools`` are either a registry reference (resolved by name) or an
inline HTTP tool. The HTTP compiler fills ``{placeholder}`` tokens in url/headers
from the model's args and strips them from the request body, so a secret passed in
a header never leaks into the payload.
"""
...
from lg_agent.core.config import HttpToolSpec, RegistryToolSpec, ToolSpec
from lg_agent.core.registry import TOOLS
