...
``AgentMiddleware``. A config references them by name, e.g.
...
# =============================================================================
# max_turns — cap model calls per run
# =============================================================================
...
class LogToolCallsMiddleware(AgentMiddleware):
    ...
    Implements BOTH sync and async variants: a sub-agent runs sync (via the re-pump
    ``stream``), but the orchestrator runs under ``astream`` and would otherwise hit
    "awrap_tool_call not available".
    """
