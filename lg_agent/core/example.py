...
This is the contract :mod:`lg_agent.core.config` is designed around. It is a
*living doc*, not a runnable agent: it references registry tools
(``check_order_status`` / ``create_support_ticket``) that the shopping platform
doesn't register, so it always *validates* but only *builds* once those tools are
stubbed. It exists to show the full JSON surface of an on-the-fly agent.
"""
...
EXAMPLE_AGENT_CONFIG = {
    ...
    "instructions": ["Be concise and empathetic.", "Respond in English."],
    "model": {"provider_model": "openai:gpt-5-mini", "temperature": 0.0},
    "skills": [
        {
            "kind": "custom",
            "name": "checkout_flow",
            "description": "use when the user asks for checkout",
            "skill": "long text",
        }
    ],
    "tools": [
