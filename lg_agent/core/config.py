"""The declarative agent config — one ``AgentConfig`` describes one agent.

This is the JSON contract a tenant writes to spin up an *on-the-fly* agent. It is
pure data: JSON-serializable, ``model_dump()`` / ``model_validate()`` round-trips,
and ``extra="forbid"`` everywhere so a typo'd key fails loudly at parse time
instead of silently doing nothing.

A config references platform capabilities **by name** (tools, skills, guardrails,
middleware) — those names are resolved against the registries in
:mod:`lg_agent.core.registry`. The only things that are NOT config are the runtime
wiring args (``context_schema`` / ``store`` / ``delegates``) passed to
:func:`lg_agent.core.builder.build_agent`.
"""
...
class ModelConfig(BaseModel):
    ...
    (see :func:`lg_agent.core.model.default_provider_model`).
    """
...
# =============================================================================
# Tools — a discriminated union on "kind"
# =============================================================================
...
# =============================================================================
# Skills — a discriminated union on "kind"
# =============================================================================
...
class GuardrailSpec(BaseModel):
    """A safety rule resolved via :data:`lg_agent.core.registry.GUARDRAILS`.
