...
from lg_agent.shopping.domain.cart import CartItem
...
def search(query: str, limit: int = 5) -> list[Product]:
    ...
    Tokenize both sides and require exact token equality or a proper-prefix match of
    length >= 3 (substring scoring let "ca" match "cap", which cascaded into spurious
    results for serviceability questions).
    """
