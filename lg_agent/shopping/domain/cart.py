"""Cart model with step tracking, freshness checks, and an invariant gate.

Three principles:

1. **The step tracker drives forward progress.** ``Cart.step`` is a derived
   property telling the checkout sub-agent which field to capture next.
2. **Derived values carry the fingerprint of their inputs.** Shipping/tax store the
   (items, zip, delivery_option) tuple they were quoted for; any mutation to those
   inputs makes the quote stale.
3. **Blockers gate confirmation.** ``Cart.blockers()`` is the invariant safety net:
   confirmation refuses while it is non-empty, so ``cart.confirmed`` (not model
