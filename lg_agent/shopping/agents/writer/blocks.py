...
accumulated :class:`~lg_agent.core.step.StepResult` list + the live cart, with
ids/prices copied verbatim from what the sub-agents produced. The LLM never writes
these, so it cannot hallucinate a product id or a total — this is the hallucination
firewall, and it is why the writer can stream prose freely (the facts are grounded).
"""
...
from lg_agent.shopping.domain.orders import Order, get_order
