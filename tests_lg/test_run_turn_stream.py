...
from lg_agent.core.guardrails import compile_input_rules
...
def test_routing_memo_merges_live_state_and_persisted_recalls():
    ...
    from lg_agent.shopping.agents.orchestrator.routing import build_memo
    from lg_agent.shopping.domain import CartService

    cs = CartService()
    cs.add_item("P-2")
    memo = build_memo(
        ShoppingContext(cart_service=cs),
        {"product_rec": "Recently shown products: P-4 Green Cap $14.50"},
    )
...
def test_absorb_recalls_persists_step_recall_by_sop():
    # the orchestrator's memory keys opaque recall text by sop, never inspecting it
    from lg_agent.shopping.agents.orchestrator.routing import absorb_recalls

    ctx = ShoppingContext()
    ctx.step_results.append(StepResult(sop="order_status", recall="Recently looked up order: ORD-9 delivered"))
    notes: dict[str, str] = {}
    absorb_recalls(ctx, notes)
    assert notes == {"order_status": "Recently looked up order: ORD-9 delivered"}
