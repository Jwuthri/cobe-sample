"""Shared fixtures for tests_lg. No test makes a real LLM call."""
...
def _reset_counters(monkeypatch):
    ...
    monkeypatch.setattr(
        "lg_agent.shopping.domain.cart_service._CART_COUNTER", itertools.count(1000)
    )
    monkeypatch.setattr(
        "lg_agent.shopping.domain.cart_service._RECEIPT_COUNTER", itertools.count(9000)
    )
    yield
