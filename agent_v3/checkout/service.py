"""All cart mutations go through ``CartService``.

Each mutator:
  1. Updates the targeted fields.
  2. Invalidates any derived quotes whose inputs changed.
  3. Returns a short human-readable event string.

This is where the invalidation *policy* lives. Tools should be dumb
...
from agent_v3.checkout.cart import (
    Cart,
    DeliveryOption,
    PaymentMethod,
    PromoApplication,
...
from agent_v3.checkout.catalog import get as get_product
from agent_v3.checkout.pricing import quote_promo, quote_shipping, quote_tax
from agent_v3.checkout.serviceability import lookup as lookup_zip
...
class CartService:
    ...
    def set_address(
        ...
        if prev_zip != zip_code:
            ...
            # If the previously-picked delivery isn't valid for the new zip,
            # clear it — we'll re-ask after serviceability check.
    ...
    def confirm(self) -> str:
        ...
        return f"Confirmed! Receipt {self.cart.receipt_id}. " f"Total ${self.cart.grand_total:.2f}."
