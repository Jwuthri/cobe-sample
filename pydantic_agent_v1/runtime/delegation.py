...
writes the common skeleton once:
  ...
  3. run the worker agent on the orchestrator's self-contained ``query`` — passing
     the SHARED ``deps`` (one live cart) and threading ``usage`` so token counts add
     up across the whole turn;
...
class Worker:
    ...
    The ``agent`` is a plain Pydantic AI ``Agent``; the hooks translate between the
    ...
    agent: Agent  # the Pydantic AI sub-agent
...
async def run_subagent(ctx: RunContext[ShoppingDeps], worker: Worker, query: str) -> str:
    ...
    result = await worker.agent.run(query, deps=deps, usage=ctx.usage)
        ...
        if isinstance(m, ModelRequest):
            for part in m.parts:
                if isinstance(part, ToolReturnPart):
                    out.append(ToolReturn(part.tool_name, str(part.content)))
    return out
        ...
        if isinstance(m, ModelResponse):
            for part in m.parts:
                if isinstance(part, ToolCallPart):
                    out.append(events.tool_start(part.tool_name, _args_dict(part)))
        elif isinstance(m, ModelRequest):
            for part in m.parts:
                if isinstance(part, ToolReturnPart):
                    out.append(events.tool_end(part.tool_name, str(part.content)))
    return out
