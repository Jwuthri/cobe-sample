...
from pydantic_agent_v1.agents import (
    BLOCK_BY_SOP,
    ROUTER_PROMPT,
    WORKERS,
    WRITER_SYSTEM,
...
from pydantic_agent_v1.runtime import ShoppingDeps
from pydantic_agent_v1.runtime import events, trace
from pydantic_agent_v1.runtime.guardrails import InputRule, run_input_guardrails
from pydantic_agent_v1.snapshot import build_snapshot
...
class ShoppingSession:
    ...
    async def run_turn_stream(self, user_text: str) -> AsyncGenerator[dict, None]:
        ...
        history = _to_message_history(prior)
        task = asyncio.create_task(self._run_orchestrator(outcome.text, deps, history, bus))
        while True:
            ev = await bus.get()
            if ev is _DONE:
                break
    ...
    async def _run_orchestrator(
        self, user_text: str, deps: ShoppingDeps, history: list[ModelMessage], bus: asyncio.Queue
    ) -> None:
        """Run the router; its tool wrappers push UI events onto the bus as they go."""
        try:
            await self.orchestrator_agent.run(user_text, deps=deps, message_history=history)
        except Exception as e:  # noqa: BLE001
    ...
    async def _stream_writer(self, payload_json: str) -> AsyncGenerator[dict, None]:
        ...
        for _attempt in (1, 2):
            parts: list[str] = []
            async with self.writer_agent.run_stream(payload_json) as result:
...
# --------------------------------------------------------------------------- #
# transcript → Pydantic AI message history (clean text only; no tool-call noise)
# --------------------------------------------------------------------------- #
def _to_message_history(transcript: list[dict]) -> list[ModelMessage]:
