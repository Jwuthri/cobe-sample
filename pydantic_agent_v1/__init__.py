"""pydantic_agent_v1 — the agent_v4_1 shopping assistant, rebuilt from scratch on
Pydantic AI.
...
streaming. The architecture is three clean layers:
  ...
  * :mod:`pydantic_agent_v1.domain`  — the store, as pure logic (the behavioral spec);
  * :mod:`pydantic_agent_v1.runtime` — thin generic glue (deps, events, delegation);
  * :mod:`pydantic_agent_v1.agents`  — five agents, one self-contained file each.
...
from pydantic_agent_v1.session import ShoppingSession  # noqa: E402
