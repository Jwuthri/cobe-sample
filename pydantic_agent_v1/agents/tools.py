...
Each function is a Pydantic AI tool. Stateless lookups (catalog / serviceability)
are plain functions; anything that touches the cart takes ``ctx: RunContext[
ShoppingDeps]`` and reads the one shared :class:`CartService` off ``ctx.deps``.

The agent files pick which of these they expose (see each ``Agent(tools=[...])``).
...
from pydantic_agent_v1.domain import catalog
from pydantic_agent_v1.domain.cart_service import CartError
from pydantic_agent_v1.domain.memory import (
    recent_orders,
    remember_address,
    remember_order,
    remember_payment,
)
from pydantic_agent_v1.domain.orders import ORDERS, get_order
from pydantic_agent_v1.domain.serviceability import lookup as lookup_zip
...
# cart edits
# =========================================================================== #
def add_item(ctx: RunContext[ShoppingDeps], product_id: str, quantity: int = 1) -> str:
    """Add a product (e.g. 'P-1') to the cart with the given quantity (default 1)."""
    try:
        return ctx.deps.cart_service.add_item(product_id, quantity)
    except CartError as e:
        return f"error: {e}"


def remove_item(ctx: RunContext[ShoppingDeps], product_id: str) -> str:
    """Remove a product line from the cart."""
    try:
        return ctx.deps.cart_service.remove_item(product_id)
    except CartError as e:
        return f"error: {e}"


def set_quantity(ctx: RunContext[ShoppingDeps], product_id: str, quantity: int) -> str:
    """Set the quantity of a cart line. quantity=0 removes the line."""
    try:
        return ctx.deps.cart_service.set_quantity(product_id, quantity)
    except CartError as e:
        return f"error: {e}"


def get_cart_summary(ctx: RunContext[ShoppingDeps]) -> str:
    """Return a multi-line summary of the cart, including step + blockers."""
    c = ctx.deps.cart_service.cart
    lines = [
...
def set_customer(
    ...
    """
    try:
        return ctx.deps.cart_service.set_customer(first_name, last_name, email)
    except CartError as e:
        return f"error: {e}"
...
def set_address(
    ...
    return ctx.deps.cart_service.set_address(street, city, zip_code, state=state, country=country)


def lookup_serviceability(ctx: RunContext[ShoppingDeps]) -> str:
    """Check whether the saved address is serviceable; sets serviceable_options."""
    try:
        return ctx.deps.cart_service.lookup_serviceability()
    except CartError as e:
        return f"error: {e}"
...
def set_delivery_option(
    ...
    try:
        return ctx.deps.cart_service.set_delivery_option(option)
    except CartError as e:
        return f"error: {e}"


def quote_shipping(ctx: RunContext[ShoppingDeps]) -> str:
    """Compute and store a shipping quote for the current cart inputs."""
    try:
        return ctx.deps.cart_service.quote_shipping()
    except CartError as e:
        return f"error: {e}"


def compute_tax(ctx: RunContext[ShoppingDeps]) -> str:
    """Compute and store tax for the current cart."""
    try:
        return ctx.deps.cart_service.compute_tax()
    except CartError as e:
        return f"error: {e}"


def apply_promo(ctx: RunContext[ShoppingDeps], code: str) -> str:
    """Apply a promo code (e.g. WELCOME10, SHOES20)."""
    try:
        return ctx.deps.cart_service.apply_promo(code)
    except CartError as e:
        return f"error: {e}"
...
def attach_payment(
    ...
    try:
        return ctx.deps.cart_service.attach_payment(method, card_token=card_token)
    except CartError as e:
        return f"error: {e}"


def confirm_checkout(ctx: RunContext[ShoppingDeps]) -> str:
...
# order status
# =========================================================================== #
def get_order_status(ctx: RunContext[ShoppingDeps], order_id: str) -> str:
...
def list_recent_orders(ctx: RunContext[ShoppingDeps], limit: int = 5) -> str:
