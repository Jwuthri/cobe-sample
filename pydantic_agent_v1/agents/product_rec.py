...
Everything that defines this worker lives here: its prompt, the Pydantic AI agent
...
hooks that bridge a run to the shared cart + the writer:
  ...
  * ``_cart_note``  — a dynamic instruction injecting the current cart (so it can
    edit lines without re-searching);
...
from pydantic_agent_v1.agents.names import CHECKOUT, PRODUCT_REC
from pydantic_agent_v1.runtime import MODEL_NAME, ShoppingDeps, StepResult, Worker, settings, tool_returns
...
@agent.instructions
def _cart_note(ctx: RunContext[ShoppingDeps]) -> str:
    """Show the worker the live cart so it can edit lines without re-searching."""
    cart = ctx.deps.cart_service.cart
    if not cart.items:
        return ""
