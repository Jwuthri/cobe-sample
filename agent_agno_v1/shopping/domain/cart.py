...
Self-contained copy of the shopping cart model (this package does not import any
other agent_* package). Identical business rules to agent_v4_1 — only the import
paths change. The framework port (LangGraph → Agno) lives entirely above the
domain; the domain is pure Python.

Three principles:

1. **Step tracker drives forward progress.** ``Cart.step`` is a derived
   property telling the checkout member which field to capture next.
