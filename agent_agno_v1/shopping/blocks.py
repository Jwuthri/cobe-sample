...
verbatim from what the members produced. The LLM never writes these, so it cannot
hallucinate a product id or a total — this is the hallucination firewall, and it
is what lets the supervisor stream its prose freely (the facts are grounded).
"""
...
from agent_agno_v1.shopping.domain.orders import Order, get_order
...
def build_blocks(
    ...
    ``block_by_sop`` maps a member name to the block kind it produces
