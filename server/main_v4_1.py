"""FastAPI bridge for ``agent_v4_1`` — the SAME web UI that v2/v4/v5 use.

It speaks the v4/v5 SSE/JSON contract, **plus** two backward-safe additions the
streaming architecture needs:

  * ``{type:"token", content}``     — one per writer delta (live token streaming);
  * ``{type:"guardrail", stage, rule, action}`` — an input-guardrail trip.

Unlike v4 (node-by-node, validate-then-emit) and v5 (whole turn blocks, then
synthetic events), v4_1 is async end-to-end: ``run_turn_stream`` yields events as
they happen, and the writer's tokens reach the client as it generates them. The
existing frontend keeps working even without the optional token-rendering patch
...
app = FastAPI(title="agent_v4_1 (streaming) debug API", version="0.1.0")
...
def health() -> dict[str, str]:
    return {"status": "ok", "engine": "agent_v4_1"}
