"""FastAPI bridge for ``google_adk_agent_v1`` — speaks the same SSE/JSON contract as
...
Point the web UI at it with ``AGENT_V2_API_URL=http://localhost:8005`` (or run it on
...
app = FastAPI(title="google_adk_agent_v1 (streaming) debug API", version="0.1.0")
...
def health() -> dict[str, str]:
    return {"status": "ok", "engine": "google_adk_agent_v1"}
