"""FastAPI bridge for ``pydantic_agent_v1`` — speaks the same SSE/JSON contract as
...
Point the web UI at it with ``AGENT_V2_API_URL=http://localhost:8003`` (or run it on
...
app = FastAPI(title="pydantic_agent_v1 (streaming) debug API", version="0.1.0")
...
def health() -> dict[str, str]:
    return {"status": "ok", "engine": "pydantic_agent_v1"}
