"""FastAPI bridge for ``agent_openai_sdk_v1`` — speaks the same SSE/JSON contract as
the other engines, so the existing web UI works unchanged.
...
Point the web UI at it with ``AGENT_V2_API_URL=http://localhost:8004`` (or run it on
...
from agent_openai_sdk_v1 import ShoppingSession

app = FastAPI(title="agent_openai_sdk_v1 (streaming) debug API", version="0.1.0")
...
def health() -> dict[str, str]:
    return {"status": "ok", "engine": "agent_openai_sdk_v1"}
