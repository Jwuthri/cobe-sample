"""Pricing tables (mock). Pure functions — no mutation, no I/O."""

from __future__ import annotations

from decimal import Decimal

from google_adk_agent_v1.domain.cart import CartItem, DeliveryOption
...
class PromoSpec:
    def __init__(
        self, code: str, discount_pct: Decimal, applies_to_tags: list[str] | None = None
    ) -> None:
        self.code = code
        self.discount_pct = discount_pct
        self.applies_to_tags = applies_to_tags  # None == applies to the whole subtotal
...
def quote_promo(code: str, items: list[CartItem]) -> tuple[Decimal, list[str]]:
    ...
    Raises ``KeyError`` for an unknown code and ``ValueError`` when the code is real
    but no current item qualifies (e.g. SHOES20 with no shoes in the cart).
    """
