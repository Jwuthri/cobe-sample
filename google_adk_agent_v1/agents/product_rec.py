...
Everything that defines this worker lives here: its prompt, the ADK ``LlmAgent``
...
hooks that bridge a run to the shared cart + the writer:
  ...
  * ``_instruction`` — an ADK instruction provider injecting the prompt + current cart
    (so it can edit lines without re-searching);
...
from google_adk_agent_v1.agents.names import CHECKOUT, PRODUCT_REC
from google_adk_agent_v1.runtime import ShoppingDeps, StepResult, Worker, deps_from, gen_config, make_model, tool_returns
...
def _instruction(ctx: ReadonlyContext) -> str:
    ...
    if key is None:
        return PROMPT
    return PROMPT + _cart_note(registry.get(key))
...
def extract(deps: ShoppingDeps, run_events, before) -> StepResult:
    cart = deps.cart_service.cart
    before = before or {}
    returns = tool_returns(run_events)
