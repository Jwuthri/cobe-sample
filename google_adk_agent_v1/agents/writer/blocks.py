"""The writer's structured output — typed "rich reply" cards, built deterministically.

The cards (product list / order status / checkout summary) are assembled from the
accumulated :class:`StepResult` list + the live cart, with ids/prices copied verbatim
from what the workers produced. The LLM never writes these, so it cannot hallucinate
a product id or a total — this is the hallucination firewall, and it is why the writer
can stream prose freely (the facts beside it are already grounded).
"""
...
from google_adk_agent_v1.domain.orders import Order, get_order
from google_adk_agent_v1.runtime import StepResult
...
def _order_from_raw(raw: str | None) -> Order | None:
    ...
    return get_order(match.group(1)) if match else None
...
# assembly
# =========================================================================== #
def build_blocks(step_results: list[StepResult], cart, block_by_sop: dict[str, str | None]) -> list[dict]:
    """Assemble the turn's typed blocks from step results + cart.

    ``block_by_sop`` maps a worker name to the block kind it produces. Conversational
    turns (no worker ran) yield ``[]``.
    """
    blocks: list = []
    checkout_done = False
    for sr in step_results:
        ...
        if kind == "product_reco":
            ...
            serv_raw = serv.get("raw") if isinstance(serv, dict) else serv if isinstance(serv, str) else None
            if products or added or serv_raw:
                blocks.append(ProductRecoBlock(products=products, added_ids=added, serviceability=serv_raw))
