...
In Pydantic AI this object rode in ``ctx.deps``; in Google ADK it lives in a
process-level :mod:`registry` and tools resolve it from a string key in session
state (``registry.get(tool_context.state[RUNTIME_KEY])``). Either way it is the SAME
instance from the orchestrator into every delegated sub-agent run, so one live
:class:`~google_adk_agent_v1.domain.CartService` is mutated in place across the whole
...
* ``lock`` — serializes worker runs so a compound message mutates the one cart and the
  one event bus sequentially (ADK's analogue of ``parallel_tool_calls=False``).
"""
...
class ShoppingDeps:
    ...
    debug: bool = True
    # Serializes delegated worker runs so the shared cart + event bus stay race-free
    # even if the router emits several tool calls in one model step.
    ...
    def routing_context(self) -> dict[str, str]:
        ...
        """
        from google_adk_agent_v1.domain import CheckoutStep
