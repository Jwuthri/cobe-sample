"""Long-term memory — InMemoryStore + namespace helpers.

Production would swap ``InMemoryStore`` for a SQLite or Postgres-backed
store; the interface is identical (``put`` / ``get`` / ``search``).
"""
