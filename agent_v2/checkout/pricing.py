"""Pricing tables (stubs).

Realistic enough to demo invalidation; not a real billing engine.
Keep this module pure-functional — no mutation.
"""
...
from agent_v2.checkout.cart import CartItem, DeliveryOption

# ------ shipping ------
# key: (zip prefix, delivery_option) -> (cost, eta_hours)
...
# ------ tax ------
# key: zip prefix -> rate. Default 0.05.
...
def quote_promo(code: str, items: list[CartItem]) -> tuple[Decimal, list[str]]:
    """Return (discount_amount, list_of_skus_the_discount_was_computed_against).

    Raises ``KeyError`` for unknown codes, ``ValueError`` when the code
    matches but no current items qualify (e.g. SHOES20 with no shoes).
    """
