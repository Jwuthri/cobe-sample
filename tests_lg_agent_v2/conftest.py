"""Shared fixtures for tests_lg_agent_v2. No test makes a real LLM call.
...
def _reset_counters(monkeypatch):
    ...
    monkeypatch.setattr("lg_agent_v2.domain.cart_service._CART_COUNTER", itertools.count(1000))
    monkeypatch.setattr("lg_agent_v2.domain.cart_service._RECEIPT_COUNTER", itertools.count(9000))
    yield
...
def make_session():
    ...
    from lg_agent_v2.agents import build_writer
    from lg_agent_v2.agents import checkout as co
    from lg_agent_v2.agents import order_status as os_
    from lg_agent_v2.agents import product_rec as pr
    from lg_agent_v2.agents import writer as default_writer
    from lg_agent_v2.agents.orchestrator import build_orchestrator
    from lg_agent_v2.agents.orchestrator import orchestrator as default_orch
    ...
    def _make(
        ...
        **session_kwargs,
        ...
        if product_rec is not None:
            worker_agents["product_rec"] = pr.build(product_rec)
        if checkout is not None:
            worker_agents["checkout"] = co.build(checkout)
        if order_status is not None:
            worker_agents["order_status"] = os_.build(order_status)

        if orchestrator is not None or worker_agents:
            orch = build_orchestrator(model=orchestrator, worker_agents=worker_agents)
        else:
            orch = default_orch
        wr = build_writer(writer) if writer is not None else default_writer
