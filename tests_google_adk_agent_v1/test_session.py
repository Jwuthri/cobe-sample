"""End-to-end pipeline tests driven by scripted models (no real LLM).
...
from tests_google_adk_agent_v1.conftest import ScriptedModel, call_then_done, say, sequence

from google_adk_agent_v1 import ShoppingSession
...
def test_empty_cart_hides_checkout_tool(override):
    """The empty-cart guard (before_model_callback) removes checkout from the router's tools."""
    seen: dict[str, list[str]] = {}

    def _capture_tools(llm_request):
        names = []
    ...
    with override(orchestrator=ScriptedModel(_capture_tools), writer=say("Hi.")):
        s.run_turn("hello")
...
def test_input_guardrail_blocks_before_any_model_call(override):
    from google_adk_agent_v1.runtime.guardrails import Blocklist
