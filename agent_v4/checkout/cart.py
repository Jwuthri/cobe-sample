...
1. **Step tracker drives forward progress.** ``Cart.step`` is a derived
   property that tells the checkout subagent which sub-skill it should
   be loading next (collect_identity → collect_address → …).

2. **Derived values carry the fingerprint of their inputs.** Shipping
   and tax store the (items, zip, delivery_option) tuple they were
