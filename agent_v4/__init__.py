"""agent_v4 — LangChain v1 + LangGraph hybrid agent pipeline.
...
CWD so ``python -m agent_v4.cli`` and ``langgraph dev`` both work
...
# 1) package-local .env (when running from the agent_v4 dir)
