...
Identical contract to agent_v4_1 (this is a clean-room rebuild on the OpenAI
Agents SDK, so the public schema a tenant writes is unchanged). It is pure data:
...
What it intentionally keeps out of scope:
  ...
  * ``ModelConfig.provider_model`` (a ``"provider:model"`` string) — the factory
    maps it onto the OpenAI Agents SDK model id (stripping the ``openai:`` prefix).

Runtime concerns (``context``/``store``) are NOT config fields — they're passed to
:func:`openai_agent_v1.core.factory.build_agent`, keeping the config serializable.
"""
...
class ModelConfig(BaseModel):
    ...
    ``None`` resolves to the env default (see
    :func:`openai_agent_v1.core.models.default_provider_model`).
    """
...
class GuardrailSpec(BaseModel):
    """A safety rule resolved via :data:`openai_agent_v1.core.registry.GUARDRAILS`.
