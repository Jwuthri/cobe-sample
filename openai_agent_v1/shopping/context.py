...
Extends the generic :class:`openai_agent_v1.core.context.TurnContext` with a
``cart_service`` handle. The same instance flows orchestrator → sub-agent tool →
sub-agent run, so every tool mutates one shared cart within a turn.

Field names are a superset of what the shopping tools read
(``wrapper.context.cart_service`` / ``wrapper.context.user_id``), so the tools
...
from openai_agent_v1.core.context import TurnContext
