"""Pricing tables (stubs). Pure-functional, no mutation. Self-contained copy."""

from __future__ import annotations

from decimal import Decimal

from openai_agent_v1.shopping.domain.cart import CartItem, DeliveryOption
...
def quote_promo(code: str, items: list[CartItem]) -> tuple[Decimal, list[str]]:
    ...
    Raises ``KeyError`` for unknown codes, ``ValueError`` when the code matches
    but no current items qualify (e.g. SHOES20 with no shoes).
    """
