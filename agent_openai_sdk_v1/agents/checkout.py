...
to *remember* anything: the cart is the truth. Every run, a dynamic instructions
callable injects a deterministic "what's done / what's next" block rendered from
:pyattr:`Cart.step`. In the LangChain build this was a bespoke ``cart_anchor``
middleware; in the SDK it is just ``Agent(instructions=callable)`` — re-evaluated
on every run, so a mid-turn mutation updates it for free.
...
from agent_openai_sdk_v1.agents.names import CHECKOUT
from agent_openai_sdk_v1.runtime import MODEL_NAME, ShoppingContext, StepResult, Worker, settings
...
def _instructions(wrapper: RunContextWrapper[ShoppingContext], agent: Any) -> str:
    ...
    return _BASE_PROMPT + "\n\n" + checkout_progress(wrapper.context.cart_service.cart)
...
def _enabled_when_cart_non_empty(
    wrapper: RunContextWrapper[ShoppingContext], _agent: Any
) -> bool:
    """Empty-cart guard: hide the checkout delegate from the orchestrator while empty."""
    return bool(wrapper.context.cart_service.cart.items)
...
WORKER = Worker(
    ...
    is_enabled=_enabled_when_cart_non_empty,
)
