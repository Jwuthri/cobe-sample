...
Each function is a SDK ``@function_tool``. Stateless lookups (catalog /
serviceability) are plain functions; anything that touches the cart takes
``wrapper: RunContextWrapper[ShoppingContext]`` and reads the one shared
:class:`CartService` off ``wrapper.context``.
...
from agent_openai_sdk_v1.domain import catalog
from agent_openai_sdk_v1.domain.cart_service import CartError
from agent_openai_sdk_v1.domain.memory import (
    recent_orders,
    remember_address,
    remember_order,
    remember_payment,
)
from agent_openai_sdk_v1.domain.orders import ORDERS, get_order
from agent_openai_sdk_v1.domain.serviceability import lookup as lookup_zip
...
# =========================================================================== #
@function_tool
def add_item(wrapper: RunContextWrapper[ShoppingContext], product_id: str, quantity: int = 1) -> str:
    """Add a product (e.g. 'P-1') to the cart with the given quantity (default 1)."""
    try:
        return wrapper.context.cart_service.add_item(product_id, quantity)
...
def remove_item(wrapper: RunContextWrapper[ShoppingContext], product_id: str) -> str:
    ...
    try:
        return wrapper.context.cart_service.remove_item(product_id)
    except CartError as e:
        return f"error: {e}"


@function_tool
def set_quantity(wrapper: RunContextWrapper[ShoppingContext], product_id: str, quantity: int) -> str:
    """Set the quantity of a cart line. quantity=0 removes the line."""
    try:
        return wrapper.context.cart_service.set_quantity(product_id, quantity)
    except CartError as e:
        return f"error: {e}"
...
def set_customer(
    ...
    """
    try:
        return wrapper.context.cart_service.set_customer(first_name, last_name, email)
    except CartError as e:
        return f"error: {e}"
...
def set_address(
    ...
    return wrapper.context.cart_service.set_address(street, city, zip_code, state=state, country=country)
...
def lookup_serviceability(wrapper: RunContextWrapper[ShoppingContext]) -> str:
    ...
    try:
        return wrapper.context.cart_service.lookup_serviceability()
    except CartError as e:
        return f"error: {e}"
...
def set_delivery_option(
    ...
    try:
        return wrapper.context.cart_service.set_delivery_option(option)
    except CartError as e:
        return f"error: {e}"
...
def quote_shipping(wrapper: RunContextWrapper[ShoppingContext]) -> str:
    ...
    try:
        return wrapper.context.cart_service.quote_shipping()
    except CartError as e:
        return f"error: {e}"
...
def compute_tax(wrapper: RunContextWrapper[ShoppingContext]) -> str:
    ...
    try:
        return wrapper.context.cart_service.compute_tax()
    except CartError as e:
        return f"error: {e}"
...
def apply_promo(wrapper: RunContextWrapper[ShoppingContext], code: str) -> str:
    ...
    try:
        return wrapper.context.cart_service.apply_promo(code)
    except CartError as e:
        return f"error: {e}"
...
def attach_payment(
    ...
    try:
        return wrapper.context.cart_service.attach_payment(method, card_token=card_token)
    except CartError as e:
        return f"error: {e}"
...
def confirm_checkout(wrapper: RunContextWrapper[ShoppingContext]) -> str:
    ...
    if service.cart.payment_method:
        remember_payment(ctx.store, ctx.user_id, service.cart.payment_method, service.cart.card_token)
    remember_order(
        ctx.store,
        ctx.user_id,
        {
