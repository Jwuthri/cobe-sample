...
writer does that). Two SDK features replace what used to be bespoke middleware:

  * **reference memo** — the agent's ``instructions`` is a callable that returns
    the static router prompt + a live cart / remembered-facts memo, so it can
    resolve "the green one" without the workers seeing the chat;
  * **empty-cart guard** — the checkout delegate's ``is_enabled`` (on its
    :func:`agents.Agent.as_tool`) returns ``False`` while the cart is empty, so an
    "add X" can never route to checkout. Structural, not prompt-enforced.
...
from agent_openai_sdk_v1.agents import checkout, order_status, product_rec
from agent_openai_sdk_v1.runtime import MODEL_NAME, ShoppingContext, build_worker_tool, settings
...
def build_memo(ctx: ShoppingContext) -> str:
    ...
    in a previous turn (kept in ``ctx.routing_notes``). Returns ``""`` when there
    is nothing to resolve against. Used both as the orchestrator's dynamic
    instruction and (read-only) by the session's debug trace.
    """
    live = ctx.routing_context()
    blocks = [text for text in live.values() if text]
    blocks += [text for key, text in ctx.routing_notes.items() if key not in live and text]
    if not blocks:
        return ""
    return (
...
def _instructions(wrapper: RunContextWrapper[ShoppingContext], agent: Any) -> str:
    return ROUTER_PROMPT + build_memo(wrapper.context)


orchestrator = Agent[ShoppingContext](
    name="orchestrator",
    model=MODEL_NAME,
    model_settings=settings(0.0, parallel_tool_calls=False),
    instructions=_instructions,
    tools=[build_worker_tool(w) for w in WORKERS],
)


def absorb_recalls(ctx: ShoppingContext) -> None:
    """Persist this turn's ``recall`` snippets so the NEXT turn's memo can use them."""
    for sr in ctx.steps:
        if sr.recall:
            ctx.routing_notes[sr.sop] = sr.recall
