"""The cart — the heart of the checkout, expressed as pure data + invariants.

Read this file first. Everything the agents do ultimately moves this object
through a small state machine and the rest of the system reports on it.

Three ideas carry the whole checkout:

1. **One derived ``step`` drives forward progress.** :pyattr:`Cart.step` is computed
   from the fields that are filled in — it answers "what does the agent capture
   next?" so the agent never has to track state in a growing chat thread.
2. **Derived quotes carry the fingerprint of their inputs.** Shipping and tax store
   the ``(items, zip, delivery)`` tuple they were quoted for, so any later edit to
   those inputs makes the quote *stale* automatically — no manual invalidation bugs.
3. **Blockers gate confirmation.** :pymeth:`Cart.blockers` is the safety net: the
   order cannot be confirmed while it is non-empty. ``cart.confirmed`` — not model
   prose — is the source of truth that an order was actually placed.
"""
...
class Customer(BaseModel):
    ...
    email: str | None = None  # optional; never gated
...
class ShippingFingerprint(BaseModel):
    """The inputs a shipping quote depends on: items + zip + delivery option."""
...
class TaxFingerprint(BaseModel):
    """The inputs a tax quote depends on: items + zip (delivery is irrelevant)."""
...
def _items_signature(items: list[CartItem]) -> str:
    """An order-insensitive hash of the (sku, qty) pairs in the cart."""
...
class Cart(BaseModel):
    ...
    # serviceability lookup result (populated by CartService.lookup_serviceability)
    ...
    # Derived quotes — each carries the fingerprint of the inputs it was quoted for.
    ...
    def grand_total(self) -> Decimal | None:
        # A grand total only exists once shipping AND tax are fresh — otherwise we
        # would quote a number we cannot stand behind.
    ...
    # ----- the single source of truth for "what next" -----
    @property
    def step(self) -> CheckoutStep:
        ...
        # A complete-but-unserviceable address is a dead end: we need a *different*
        # one, so route back to the address step (where set_address is available).
        ...
        # Everything is collected, but a cart edit (set_quantity / remove_item) may
        # have invalidated the shipping quote + tax. We cannot confirm with a stale
        # total, so this is its own step: recompute pricing before ready.
        if not (self.shipping_is_fresh() and self.tax_is_fresh()):
    ...
    def blockers(self) -> list[Blocker]:
        """Every reason the order cannot be placed right now. Empty == ready."""
        out: list[Blocker] = []
        if not self.items:
