"""``CartService`` — the only thing allowed to mutate a cart.

Every mutation updates the targeted fields, invalidates any derived quote whose
inputs changed, and returns a short human-readable event string. The invalidation
*policy* lives here so the tools that wrap these methods can stay dumb one-liners.

A mutation that is not allowed raises :class:`CartError`; the tool layer turns that
into an ``"error: ..."`` string the model can read and react to.
"""
...
from agent_openai_sdk_v1.domain.cart import (
    Cart,
    DeliveryOption,
    PaymentMethod,
    PromoApplication,
...
from agent_openai_sdk_v1.domain.catalog import get as get_product
from agent_openai_sdk_v1.domain.pricing import quote_promo, quote_shipping, quote_tax
...
class CartError(Exception):
    """Raised when a requested mutation is illegal (caught + surfaced by the tools)."""


# Words that show up in checkout prompts / routing labels but are never a person's
# name. The model sometimes confabulates an identity from them (e.g. it relabels an
...
def _looks_like_name(value: str) -> bool:
    """A cheap, deterministic 'is this plausibly a name?' gate (not real validation)."""
    v = (value or "").strip()
    if not v:
        return False
