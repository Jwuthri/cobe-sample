...
from agent_v4.graph import checkout_gate, validator
from agent_v4.state import AgentState
from langgraph.types import Command


def _state(**overrides) -> AgentState:
    return AgentState(user_id="u", session_id="s", active_sop=ids.CHECKOUT, **overrides)
...
def test_gate_bounces_hallucinated_confirm_when_cart_not_ready():
    ...
    assert cmd.update.get("step_results") == []
