"""The *tool* abstraction — constrained Agno function-tools over the domain.

Every tool is a plain function whose first concern is to fetch the shared
``ShoppingContext`` off ``run_context.dependencies["ctx"]`` and delegate to the
``CartService`` (or the catalog / order DB / memory store). The invalidation and
gating *policy* lives in the domain; these wrappers only translate a model call
into a domain call and stringify the result.

Agno injects ``run_context`` by exact parameter name and strict-validates the
remaining args (pydantic ``validate_call``) — so optional string params are typed
``str | None`` to tolerate a model that fills them with ``null``.
"""
...
from agno_agent_v1.domain import catalog
from agno_agent_v1.domain.cart_service import CartError
from agno_agent_v1.domain.memory import recent_orders, remember_address, remember_order, remember_payment
from agno_agent_v1.domain.orders import ORDERS, get_order
from agno_agent_v1.domain.serviceability import lookup as lookup_zip


def _ctx(run_context: RunContext) -> ShoppingContext:
    """Fetch the shared per-turn context the orchestrator passed as a dependency."""
    return (run_context.dependencies or {})["ctx"]


# ===========================================================================
# catalog / serviceability (stateless — product_rec)
# ===========================================================================
...
def add_item(product_id: str, run_context: RunContext, quantity: int = 1) -> str:
    ...
    try:
        return _ctx(run_context).cart_service.add_item(product_id, quantity)
...
def remove_item(product_id: str, run_context: RunContext) -> str:
    ...
    try:
        return _ctx(run_context).cart_service.remove_item(product_id)
    except CartError as e:
        return f"error: {e}"
...
def set_quantity(product_id: str, quantity: int, run_context: RunContext) -> str:
    ...
    try:
        return _ctx(run_context).cart_service.set_quantity(product_id, quantity)
    except CartError as e:
        return f"error: {e}"
...
def set_customer(
    ...
    """
    try:
        return _ctx(run_context).cart_service.set_customer(first_name, last_name, email)
    except CartError as e:
        return f"error: {e}"
...
def set_address(
    ...
    return _ctx(run_context).cart_service.set_address(
...
def lookup_serviceability(run_context: RunContext) -> str:
    ...
    try:
        return _ctx(run_context).cart_service.lookup_serviceability()
    except CartError as e:
        return f"error: {e}"
...
def set_delivery_option(
    ...
    try:
        return _ctx(run_context).cart_service.set_delivery_option(option)
    except CartError as e:
        return f"error: {e}"
...
def quote_shipping(run_context: RunContext) -> str:
    ...
    try:
        return _ctx(run_context).cart_service.quote_shipping()
    except CartError as e:
        return f"error: {e}"
...
def compute_tax(run_context: RunContext) -> str:
    ...
    try:
        return _ctx(run_context).cart_service.compute_tax()
    except CartError as e:
        return f"error: {e}"
...
def apply_promo(code: str, run_context: RunContext) -> str:
    ...
    try:
        return _ctx(run_context).cart_service.apply_promo(code)
    except CartError as e:
        return f"error: {e}"
...
def attach_payment(
    ...
    try:
        return _ctx(run_context).cart_service.attach_payment(method, card_token=card_token)
    except CartError as e:
        return f"error: {e}"
...
def confirm_checkout(run_context: RunContext) -> str:
    ...
    if store is not None:
        ...
        if service.cart.payment_method:
            remember_payment(store, ctx.user_id, service.cart.payment_method, service.cart.card_token)
        remember_order(
            store,
            ctx.user_id,
            {
...
def get_cart_summary(run_context: RunContext) -> str:
    ...
    c = _ctx(run_context).cart_service.cart
    lines = [
...
# ===========================================================================
# tool groupings consumed by the agent specs
# ===========================================================================
PRODUCT_REC_TOOLS = [
    search_products,
    get_product,
