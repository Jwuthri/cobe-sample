...
The writer composes from these grounded facts — never from raw tool output it
could misread. It takes the session's plain transcript (a list of ``{"role",
"content"}`` dicts), the turn's ``StepResult`` list, and the cart, and emits the
JSON payload + the mode the writer prompt switches on.
"""
...
from agno_agent_v1.agent.extractors import CHECKOUT
...
def build_writer_payload(
    messages: list[dict], step_results: list[StepResult], cart
