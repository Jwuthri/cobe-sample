"""Cart model: step tracking, freshness fingerprints, and the confirmation gate.

This is pure business logic — no Agno, no LangChain, no framework at all. It is
the source of truth for *correct shopping behavior*: subtotal/tax/shipping math,
the checkout step machine, and the ``blockers()`` invariant that decides whether
an order may be confirmed.

Three principles:

1. **The step tracker drives forward progress.** ``Cart.step`` is derived from
   the cart's contents and tells the checkout agent which field to capture next.
2. **Derived quotes carry the fingerprint of their inputs.** Shipping/tax store
   the (items, zip, delivery_option) signature they were quoted for; mutate an
   input and the quote goes stale automatically.
3. **Blockers gate confirmation.** ``blockers()`` is the safety net — confirming
   refuses while it is non-empty, so ``cart.confirmed`` (not model prose) is the
   single source of truth for "is the order placed".
"""
...
class Cart(BaseModel):
    ...
    def step(self) -> CheckoutStep:
        ...
        # have invalidated the shipping quote and tax. We CANNOT confirm with a
