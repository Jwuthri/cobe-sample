"""``CartService`` — the only thing that mutates a cart.

Each mutator (1) updates fields, (2) invalidates any derived quote whose
inputs changed, and (3) returns a short human-readable event string. Tools
are dumb wrappers over these methods, so the invalidation *policy* lives in
exactly one place.
"""
...
from agent_deepagent_v4.domain.cart import (
    Cart,
    DeliveryOption,
    PaymentMethod,
    PromoApplication,
...
from agent_deepagent_v4.domain.catalog import get as get_product
from agent_deepagent_v4.domain.pricing import quote_promo, quote_shipping, quote_tax
from agent_deepagent_v4.domain.serviceability import lookup as lookup_zip
...
class CartError(Exception):
    """Raised when a requested mutation is illegal."""
...
class CartService:
    ...
    # ----- address -----
    def set_address(
        self, street: str, city: str, zip_code: str, state: str | None = None, country: str = "US"
    ) -> str:
        ...
        if prev_zip != zip_code:
            # Changing the zip invalidates serviceability, shipping, tax, delivery.
            self.cart.shipping = None
            self.cart.tax = None
            self.cart.serviceable = None
            self.cart.serviceable_options = []
            self.cart.delivery_option = None
        return f"Address set: {street}, {city} {zip_code} {country}. Serviceability/shipping/tax invalidated."
    ...
    def lookup_serviceability(self) -> str:
        ...
        return f"Zip {self.cart.address.zip_code} is serviceable. Options: {result.options}."
    ...
    def quote_shipping(self) -> str:
        ...
        cost, eta = quote_shipping(self.cart.address.zip_code, self.cart.delivery_option, self.cart.subtotal)
        self.cart.shipping = ShippingQuote(cost=cost, eta_hours=eta, quoted_for=self.cart.shipping_fingerprint())
    ...
    def apply_promo(self, code: str) -> str:
        ...
        self.cart.promo = PromoApplication(code=code.upper(), discount=discount, applied_to_skus=skus)
    ...
    def attach_payment(self, method: PaymentMethod, card_token: str | None = None) -> str:
        ...
        notes = [f"switched from {prev or 'none'} to {method}"] if prev != method else [f"payment {method}"]
