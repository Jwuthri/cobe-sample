"""Pricing tables (stubs): shipping, tax, promos. Pure-functional, no mutation."""
...
if TYPE_CHECKING:
    from agent_deepagent_v4.domain.cart import CartItem, DeliveryOption

# ------ shipping ------
SHIPPING_TIERS: dict[tuple[str, str], tuple[Decimal, int]] = {
...
DEFAULT_SHIPPING_BY_OPTION: dict[str, tuple[Decimal, int]] = {
    "2h": (Decimal("29.99"), 2),
    "4h": (Decimal("14.99"), 4),
...
def quote_shipping(zip_code: str, option: "DeliveryOption", subtotal: Decimal) -> tuple[Decimal, int]:
    """Return (cost, eta_hours). Free standard shipping over $100."""
    prefix = zip_code[:3]
...
# ------ promo ------
class PromoSpec:
    def __init__(self, code: str, discount_pct: Decimal, applies_to_tags: list[str] | None = None) -> None:
        self.code = code
        self.discount_pct = discount_pct
        self.applies_to_tags = applies_to_tags  # None == entire subtotal
...
def quote_promo(code: str, items: "list[CartItem]") -> tuple[Decimal, list[str]]:
    """Return (discount_amount, skus_discounted). Raises KeyError (unknown code)
    or ValueError (code valid but no current items qualify)."""
