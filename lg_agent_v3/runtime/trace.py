...
def render_messages(messages: Any) -> list[dict]:
    """Render a LangChain message list into compact, JSON-safe rows for the UI."""
    from langchain_core.messages import AIMessage, HumanMessage, SystemMessage, ToolMessage

    rows: list[dict] = []
    for m in messages or []:
        if isinstance(m, SystemMessage):
            rows.append({"role": "system", "content": _trim(_text(m.content))})
        elif isinstance(m, HumanMessage):
            rows.append({"role": "human", "content": _trim(_text(m.content))})
        elif isinstance(m, ToolMessage):
            rows.append({"role": "tool", "name": m.name, "content": _trim(_text(m.content))})
        elif isinstance(m, AIMessage):
            for tc in m.tool_calls or []:
                rows.append(
                    {"role": "ai", "content": "", "tool_calls": [{"name": tc["name"], "args": tc.get("args", {})}]}
                )
            text = _text(m.content)
            if text:
                rows.append({"role": "ai", "content": _trim(text)})
    return rows
