"""lg_agent_v3 — the pydantic_agent_v1 shopping assistant, rebuilt on LangChain +
LangGraph.
...
Pydantic AI to LangChain ``create_agent`` graphs:
  ...
  * :mod:`lg_agent_v3.domain`  — the store, as pure logic (the behavioral spec);
  * :mod:`lg_agent_v3.runtime` — thin generic glue (deps/context, events, delegation,
    the middleware primitives that stand in for Pydantic AI natives);
  * :mod:`lg_agent_v3.agents`  — five agents, one self-contained file each.
...
from lg_agent_v3.session import ShoppingSession  # noqa: E402
