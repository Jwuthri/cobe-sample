...
from lg_agent_v3.agents.names import CHECKOUT, PRODUCT_REC
...
def build(model: Any | None = None, guardrails: list | None = None):
    """Compile the product_rec worker (optionally with an injected model + guardrails)."""
    return create_agent(
        ...
        # this worker can't.
        middleware=[
            dynamic_instructions(_cart_note),
            no_parallel_tools(),
            *compile_guardrails(guardrails, PRODUCT_REC),
        ],
        name=PRODUCT_REC,
    )
