...
rendered from :pyattr:`Cart.step`. In Pydantic AI this was an ``@agent.instructions``
function; here it is a :func:`~lg_agent_v3.runtime.dynamic_instructions` middleware —
re-evaluated on every model call, so a mid-turn mutation updates it for free.
...
from lg_agent_v3.agents.names import CHECKOUT
...
def _progress_anchor(deps: ShoppingDeps) -> str:
    """Inject the authoritative checkout state on every run (the cart is the truth)."""
    return checkout_progress(deps.cart_service.cart)


def build(model: Any | None = None, guardrails: list | None = None):
    """Compile the checkout worker (optionally with an injected model + guardrails)."""
    return create_agent(
        ...
        # no_parallel_tools: checkout mutates the one shared cart — serial only (same
        # reasoning as product_rec).
        middleware=[
            dynamic_instructions(_progress_anchor),
            no_parallel_tools(),
            *compile_guardrails(guardrails, CHECKOUT),
        ],
        name=CHECKOUT,
    )
